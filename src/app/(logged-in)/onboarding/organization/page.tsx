...
export default async function OrganizationOnboardingPage(props: OrganizationOnboardingPageProps) {
	...
	if (!user) {
		const href = inviteId ? `/onboarding/user?inviteId=${inviteId}` : '/onboarding/user';
		return redirect(href);
	}

	const invite = inviteId ? await getOrganizationInvitation(inviteId) : undefined;
		...
		<AnimatedLayoutContainer>
			{
				invite
					? (
						<>
							<h1 className='text-2xl text-stone-50 mb-4'>
								Fuiste invitado a <strong>{invite.organization.name}</strong>
							</h1>
							{
								invite.organization.logoUrl && (
									<Image
										src={invite.organization.logoUrl} alt={`${invite.organization.name} logo`}
										width={256} height={256} className='mx-auto h-32 mb-4'/>
								)
							}
							<p className='mb-4'>
								Fuiste invitado a unirte a <strong>{invite.organization.name}</strong> por <strong>{invite.sender.givenName} {invite.sender.familyName} </strong>.
								Al unirte serás capaz de modificar los detalles de la organización, ayudando a completar su perfil dentro de la plataforma.
							</p>
							<form action={joinOrganizationAction} className='w-full flex justify-end'>

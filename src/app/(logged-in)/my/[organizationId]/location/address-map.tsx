...
export default function AddressMap(props: AddressMapProps) {
	const {selectedCoords, mapRef, onClick} = props;
	// Required in order to fix an error with Map recreation
	const time = useMemo(() => Date.now(), []);

	return (
		<MapContainer key={time} ref={mapRef} center={[25.68, -100.31]} zoom={11} className='h-96 rounded border border-stone-800 mb-4'>
			<TileLayer
